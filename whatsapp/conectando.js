...
const  FileType  =  require ( 'tipo de archivo' )
const  fs  =  requerir ( 'fs' )
const  obtuvo  =  requiere ( 'consiguió' )
const  ruta  =  requerir ( 'ruta' )
const  PhoneNumber  =  require ( 'awesome-phonenumber' )
const  solicitud  =  requerir ( 'solicitud' )
constante  { tmpdir }  =  requerir ( 'os' )
const  util  =  require ( 'util' )
const  axios  =  requerir ( "axios" ) ;
constante  {
  Tipo de mensaje ,
  Mensaje WAMessageProto ,
  DEFAULT_ORIGIN ,
  obtenerDuraciónDeAudio ,
  MessageTypeProto ,
  MediaPathMap ,
  mimetipo ,
  MimetypeMap ,
  ...
  sha256 ,
  corriente cifrada
}  =  requerir ( '@adiwajshing/baileys' )
const  { WAConnection }  =  require ( '@adiwajshing/baileys/lib/WAConnection/0.Base' )

exportaciones _ ConexiónWA = _ConexiónWA  => {   
  clase  WAConnection  extiende  _WAConnection  {
    constructor ( ... argumentos )  {
      súper ( ... argumentos )
      if  ( ! Array . isArray ( this . _events [ 'CB:action,add:relay,message' ] ) )  this . _events [ 'CB:acción,añadir:retransmisión,mensaje' ]  =  [ este . _events [ 'CB:acción,añadir:retransmisión,mensaje' ] ]
      más  esto . _events [ 'CB:acción,añadir:retransmisión,mensaje' ]  =  [ este . _events [ 'CB:acción,añadir:retransmisión,mensaje' ] . estallar ( ) ]
      esto _ _events [ 'CB:acción,añadir:retransmisión,mensaje' ] . unshift ( función asíncrona  ( json ) {  
        prueba  {
          sea  m  =  hijo [ 2 ] [ 0 ] [ 2 ]
          if  ( m . mensaje  &&  m . mensaje . protocolMessage  &&  m . mensaje . protocolMessage . type  ==  0 )  {
            dejar  clave  =  m . mensaje _ mensaje de protocolo . llave
            Sea  c  =  esto . charlas _ obtener ( clave . remoteJid )
            Sea  a  =  c . mensajes _ dict [ ` ${ clave . identificación } | ${ clave . de mi ? 1 : 0 } ` ]
            let  participante  =  clave . de mi ? esto _ usuario _ yid : un . participante ? un . participante : clave . remotoJid
            sea  WAMSG  =  WAMessageProto . WebMessageInfo
            esto _ emit ( 'mensaje-borrar' ,  { clave , participante ,  mensaje : WAMSG . fromObject ( WAMSG . toObject ( a ) )  } )
          }
        }  atrapar  ( e )  {  }
      } )
      esto _ on ( `CB:acción, batería` ,  json  =>  {
        esto _ batería  =  Objeto . fromEntries ( Objeto . entradas ( json [ 2 ] [ 0 ] [ 1 ] ) . mapa ( v  =>  [ v [ 0 ] ,  eval ( v [ 1 ] ) ] ) )
      } )


// Alias
      esto _ sendFileFromUrl  =  esto . sendFileFromURL  =  esto . enviar archivo
    }
    
        /**
     * Reenvío de copia exacta
     * @param { Cadena } jid
     * @param { Objeto } mensaje
     * @param { Booleano } forzar avance
     * @param { Objeto } opciones
     */


async  copyNForward ( jid ,  mensaje ,  forceForward  =  falso ,  opciones  =  { } )  {
      dejar  vtype
      if  ( opciones . readViewOnce )  {
        mensaje _ mensaje  =  mensaje . mensaje  &&  mensaje . mensaje _ ephemeralMessage  &&  mensaje . mensaje _ mensaje efímero . mensaje ? mensaje _ mensaje _ mensaje efímero . mensaje : ( mensaje . mensaje  ||  indefinido )
        tipov  =  Objeto . teclas ( mensaje . mensaje . viewOnceMessage . mensaje ) [ 0 ]
        eliminar  ( mensaje . mensaje  &&  mensaje . mensaje . ignorar ? mensaje . mensaje . ignorar : ( mensaje . mensaje  ||  indefinido ) )
        borrar  mensaje . mensaje _ verOnceMessage . mensaje [ vtipo ] . ver una vez
        mensaje _ mensaje  =  {
          ... mensaje . mensaje _ verOnceMessage . mensaje
        }
      }
      let  mtype  =  Objeto . teclas ( mensaje . mensaje ) [ 0 ]
      let  content  =  esperar  esto . generarForwardMessageContent ( mensaje ,  forceForward )
      let  ctype  =  Objeto . claves ( contenido ) [ 0 ]
      sea  contexto  =  { }
      if  ( mtype  !=  MessageType . text )  context  =  message . mensaje [ mtype ] . contextInfo
      contenido [ ctipo ] . información de contexto  =  {
        ... contexto ,
        ... contenido [ ctype ] . contextInfo
      }
      const  waMessage  =  espera  esto . prepareMessageFromContent ( jid ,  contenido ,  opciones ? {
        ... contenido [ ctype ] ,
        ... opciones ,
        ... ( opciones . contextInfo ? {
          información de contexto : {
            ... contenido [ ctype ] . información de contexto ,
            ... opciones . contextInfo
          }
        } : { } )
      } : { } )
      espera  esto . reléWAMessage ( waMessage )
      devolver  waMessage
    }
...
/**
    ...
    * @param { * } mensaje
    * @param { Cadena } texto
    * @param { Cadena } remitente
    * @param { * } opciones
    * @devoluciones 
    */
    cMod ( jid ,  mensaje ,  texto  =  '' ,  remitente  =  este . usuario . jid ,  opciones  =  { } )  {
      dejar  copiar  =  mensaje . aJSON ( )
      let  mtype  =  Objeto . teclas ( copiar . mensaje ) [ 0 ]
      let  isEphemeral  =  mtype  ===  'mensaje efímero'
      si  ( es efímero )  {
        mtype  =  Objeto . teclas ( copia . mensaje . ephemeralMessage . mensaje ) [ 0 ]
      }
      let  msg  =  isEphemeral ? copiar _ mensaje _ mensaje efímero . mensaje : copiar . mensaje
      dejar  contenido  =  mensaje [ mtype ]
      if  ( tipo de contenido === 'cadena' ) msg [ mtype  ] = texto || contenido       
      else  if  ( contenido . subtítulo )  contenido . título  =  texto  ||  contenido _ subtítulo
      else  if  ( contenido.texto ) contenido . _ _ texto = texto || contenido _ texto     
      if  ( tipo de contenido ! == 'cadena' ) msg [ mtype  ] = { ... contenido , ... opciones }      
      if  ( copia . participante )  remitente  =  copia . participante  =  remitente  ||  copiar _ partícipe
      else  if  ( copia . clave . participante )  remitente  =  copia . clave _ participante  =  remitente  ||  copiar _ clave _ partícipe
      if  ( copy . key . remoteJid . include ( '@s.whatsapp.net' ) )  remitente  =  remitente  ||  copiar _ clave _ remotoJid
      else  if  ( copy . key . remoteJid . include ( '@broadcast' ) )  sender  =  sender  ||  copiar _ clave _ remotoJid
      copiar _ clave _ remoteJid  =  jid
